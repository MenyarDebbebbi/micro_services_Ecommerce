...
import iset.master.spring.repository.PersonneRepository;
...
public class JpaSpringBootApplication {
    ...
    static PersonneRepository personneRepos; // Déclaration du PersonneRepository
    ...
    public static void main(String[] args) {
        ...
        personneRepos = contexte.getBean(PersonneRepository.class); // Récupérer PersonneRepository
        ...
        // Créer un employé et l'associer à un stock
        Employe e1 = new Employe("Triki", "Samir", 1);

        // Vérifier si le stock de "Gabes" existe
        Stock sg = stockRepos.findByAdresse("Gabes");

        if (sg == null) {
            // Si le stock de "Gabes" n'existe pas, le créer
            sg = new Stock("4", "Gabes");
            stockRepos.save(sg);
        }

        // Affecter le stock à l'employé
        e1.setStock(sg);
        personneRepos.save(e1); // Sauvegarde l'employé avec PersonneRepository

        // Si un stock de "Gabes" existe, on déconnecte tous les employés qui lui sont associés et on le supprime
        if (sg != null) {
            Collection<Employe> le = sg.getEmployes();
            for (Employe e : le) {
                e.setStock(null); // Détacher l'employé du stock
                personneRepos.saveAndFlush(e); // Enregistrer l'employé
            }
            le.clear(); // Vider la liste des employés associés
            stockRepos.delete(sg); // Supprimer le stock de "Gabes"
        }
        ...
        if (le != null) {
            for (Employe e : le) {
                ...
                e.setStock(st);
                personneRepos.saveAndFlush(e); // Enregistrer l'employé avec le stock
            }
        }
    }
