...
// créer l'uri
URI uri = UriBuilder.fromUri("http://localhost:8080/produits").build();
...
Produit reponseSaveProduit = resourceSave.type(MediaType.APPLICATION_JSON).post(Produit.class, nouveauProduit);
...
Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();JsonArray jo = new JsonParser().parse(reponseGetAllProduits).getAsJsonArray();
